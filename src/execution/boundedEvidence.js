...
const ALLOWED = new Set(['evidence', 'files', 'tests', 'docs', 'facts', 'inspected','excerpts'])
...
export function normalizeApprovedEvidence(input = {}, task = '', risk = 'normal') {
  ...
  for (const value of input.excerpts || []) if (!safeRelativePath(value?.path)) return { allowed: false, reason: 'approved-evidence-unsafe-path' }
  const hasEvidence = [...ALLOWED].some(key => Array.isArray(input[key]) && input[key].length > 0)
