...
export function resolveRoleTarget(role,env=process.env){const r=cfg.roles[role];if(!r)throw new Error(`No target for ${role}`);const prefix=`HARNESS_${roleEnv[role]}`,provider=env[`${prefix}_PROVIDER`]||r.provider,model=env[`${prefix}_MODEL`]||r.model,effort=env[`${prefix}_EFFORT`]||r.effort;return{provider,model,effort,configured:Boolean(model)}}
