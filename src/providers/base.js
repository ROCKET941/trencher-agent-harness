...
export const delegateTools = [{type:'function',name:'request_context',description:'Request one bounded additional context packet.',parameters:{type:'object',properties:{paths:{type:'array',items:{type:'string'},maxItems:6},reason:{type:'string'}},required:['reason'],additionalProperties:false}},{type:'function',name:'report_result',description:'Return the completed bounded result.',parameters:{type:'object',properties:{status:{type:'string',enum:['complete','incomplete','blocked']},findings:{type:'array',items:{type:'string'}},artifact:{type:['string','null']},evidence:{type:'array',items:{type:'string'}},tests:{type:'array',items:{type:'string'}},blockers:{type:'array',items:{type:'string'}}},required:['status','findings','evidence','tests','blockers'],additionalProperties:false}}]
