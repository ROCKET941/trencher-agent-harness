...
const laneInstructions = subject => `Choose the appropriate execution environment for ${subject}, independently of model and effort. Native Luna Max only researches, reads and reports; Astra XHigh commands, integrates, runs real tests and owns final acceptance. Implementation is not reserved for Astra. External API workers receive ONLY that workstream's supplied bounded evidence and excerpts, can produce a complete candidate patch for Astra to apply and test, and cannot read files or execute tools; file paths alone are not source content. Prefer external_api for suitable bounded coding with sufficient evidence. Native workspace access needed for the overall project does not disqualify a self-contained candidate patch assignment. Prefer native_host when this assignment itself requires unavailable source or interactive tools. Do not force provider diversity. ${qualityInstructions}`
...
export async function askRoutingJev(packet, options = {}) {
  ...
  const state={task:packet.task,task_kind:kind,risk:packet.risk,root_cause_known:Boolean(packet.rootCause),root_cause:packet.rootCause,evidence:packet.evidence,excerpts:packet.excerpts,files:packet.files,tests:packet.tests,docs:packet.docs,open_questions:packet.openQuestions,facts:packet.facts,inspected:packet.inspected,workstreams,required_role:options.roleFloor||null,eligible_targets:eligible,target_efforts:targetEfforts,target_execution_modes:targetExecutionModes,registry_version:REGISTRY_VERSION,policy_version:routing.version}
  const questions = {
  ...
  const result = await askJev({ state, questions: {
    ...
    review_required: { type: 'noul', instructions: 'Assess review value. Every implementation requires one independent read-only review and native Astra XHigh final acceptance; this advice cannot waive that gate.', criteria: { true: 'Implementation or consequential investigation.', false: 'Read-only research without implementation.' } }
