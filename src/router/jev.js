...
const laneInstructions = subject => `Choose the appropriate execution environment for ${subject}, independently of model and effort. Native workers can inspect/edit the local repository and run tests. External API workers receive ONLY that workstream's supplied bounded evidence, can return analysis or a patch for the parent to apply, and cannot read files or execute tools; file paths alone are not source content. Prefer external_api only when the supplied workstream evidence is sufficient. Prefer native_host when direct workspace access is essential. Do not force provider diversity.`
function workstreamQuestions(workstreams, nativeCriteria, externalCriteria) {
  return Object.fromEntries(workstreams.flatMap((_workstream, index) => {
    ...
    return [
      ...
      [`${prefix}_native_target`,{type:'choice',instructions:`Independently, IF a native worker is used for state.workstreams[${index}], choose the cheapest capable native model. Do not depend on another answer.`,criteria:nativeCriteria}],
      ...(Object.keys(externalCriteria).length?[[`${prefix}_external_target`,{type:'choice',instructions:`Independently, IF an external worker is used for state.workstreams[${index}], choose the cheapest capable external model. Do not depend on another answer.`,criteria:externalCriteria}]]:[]),
...
export async function askRoutingJev(packet, options = {}) {
  ...
  const questions = {
    ...
    ...workstreamQuestions(workstreams,nativeCriteria,externalCriteria),
  }
