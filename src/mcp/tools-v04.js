...
export async function executeRoutedTask(input={},options={}){
  ...
  if(target.executionMode==='native_host')return{plan:planned,target,execution:null,handoff:{...plan.routingDecision.handoff,delegation:plan.delegation},reason:'native-host-agent-required'}
  if(env.HARNESS_ENABLE_PAID_EXECUTION!=='true')return{plan:planned,target,execution:null,reason:'paid-execution-owner-gate-disabled'}
