...
export function createServer(options={}) {
  ...
  const server = new McpServer({ name: 'trencher-agent-harness', version: '1.2.0' })
  server.registerTool('route_task', { description: 'Route one meaningful build phase for correctness and task fit. Jev chooses lane/model/effort for the phase and validated disjoint workstreams in one request. Use external assignment decisionIds; Luna Max is read-only research; Astra XHigh implementation stays with the permanent commander. Every implementation needs one fresh Astra XHigh final review and the commit gate.', inputSchema: z.object({ ...fields, ...routingControls, attempts: z.array(z.record(z.string(), z.unknown())).optional(), newEvidence: z.boolean().optional(), useJev: z.boolean().optional(),workspace:z.string().optional(),requestedRoute:z.object({role:z.enum(['scout','engineer','deep_debugger','reviewer','exceptional']).optional(),provider:z.enum(['openai','xai','deepseek','kimi']).optional(),model:z.string().optional(),effort:z.string().optional()}).optional() }) }, async args => out(await routeTask(args, { ...serviceOptions,useJev: options.useJev !== false && args.useJev !== false })))
  ...
  server.registerTool('check_action', { description: 'Check protected actions. Commit requires a pinned Astra XHigh final review, matching artifact digest, fresh independent acceptance and passing host-attested verification; the native host must enforce this gate before Git.', inputSchema: z.object({ action: z.string(), explicitlyAuthorized: z.boolean().default(false), commit }) }, async args => out(checkAction(args,serviceOptions)))
  ...
  server.registerTool('review_route', { description: 'Pin exactly one fresh read-only native Astra XHigh final reviewer for the complete integrated diff after verification. Supply reviewContext to bind acceptance to the artifact and commander. This fixed route requires no Jev call.', inputSchema: z.object({...fields,reviewContext:z.object({artifactDigest:digest,commanderAgentId:z.string().min(1).max(120)}).optional()}) }, async args => out(await reviewRoute(args,serviceOptions)))
