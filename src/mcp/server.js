...
export function createServer(options={}) {
  ...
  server.registerTool('route_task', { description: 'Route one meaningful build phase. Jev independently chooses execution lane, model and effort within safety floors. For external_api, call execute_routed_task with the returned decisionId only (optional budget/deadline); native_host uses a ChatGPT plan subagent. Reuse for minor follow-ups.', inputSchema: z.object({ ...fields, ...routingControls, attempts: z.array(z.record(z.string(), z.unknown())).optional(), newEvidence: z.boolean().optional(), useJev: z.boolean().optional(),workspace:z.string().optional(),requestedRoute:z.object({role:z.enum(['scout','engineer','deep_debugger','reviewer','exceptional']).optional(),provider:z.enum(['openai','xai','deepseek','kimi']).optional(),model:z.string().optional(),effort:z.string().optional()}).optional() }) }, async args => out(await routeTask(args, { ...serviceOptions,useJev: options.useJev !== false && args.useJev !== false })))
