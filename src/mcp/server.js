...
import { executeRoutedTask, resumeRoutedTask, jobStatus, taskUsage, cancelExecution, providerStatus, modelCatalog } from './tools-v04.js'
...
export function createServer(options={}) {
  ...
  const providers=options.providers||{openai:new OpenAIProvider({env}),xai:new XAIProvider({env}),deepseek:new DeepSeekProvider({env}),kimi:new KimiProvider({env})}
  for(const [name,adapter] of Object.entries(providers))registerProvider(name,adapter)
  const server = new McpServer({ name: 'trencher-agent-harness', version: '1.0.0' })
  server.registerTool('route_task', { description: 'Route to cheapest capable role.', inputSchema: z.object({ ...fields, attempts: z.array(z.record(z.string(), z.unknown())).optional(), newEvidence: z.boolean().optional(), useJev: z.boolean().optional(),workspace:z.string().optional(),requestedRoute:z.object({role:z.enum(['scout','engineer','deep_debugger','reviewer','exceptional']).optional(),provider:z.enum(['openai','xai','deepseek','kimi']).optional(),model:z.string().optional(),effort:z.string().optional()}).optional() }) }, async args => out(await routeTask(args, { ...serviceOptions,useJev: args.useJev !== false })))
