...
- One batched Jev request recommends task type, complexity, risk, worker, provider/model, effort, context, parallelism, verification, and review. Provider/model and effort are separate typed choices so confidence remains meaningful. Valid choices meeting the 0.70 floor are authoritative unless a deterministic safety or capability floor rejects them; lower-confidence advice falls back transparently to the cheapest capable native role target. Confidence still cannot justify unsafe context expansion. Jev requires the paid-execution owner gate plus a positive operator-supplied `HARNESS_JEV_CALL_COST_USD` conservative bound.
...
- Native OpenAI handoffs are explicitly `subagent-only` and `parentModelUnchanged`. Three-agent fan-out requires strong Jev parallel confidence (minimum 0.80) or three explicit validated workstreams; explicit two-way disjoint work may run as two. Ownership collisions, dependencies, ambiguous paths, and missing file/test ownership reduce execution to one worker. The host performs the actual Codex subagent spawn, integration, and verification.
