...
When releasing this change, update the desktop's installed router skill from this repository and reload the MCP tool schema (or start a fresh thread) so `execute_routed_task` exposes `decisionId`. Existing conversations with the older schema can still use legacy calls, but do not gain pinned execution until refreshed. No new credential or gate is required; `JEV_LANE_MIN_CONFIDENCE` defaults to 0.70 when unset. Keep API commander disabled.
