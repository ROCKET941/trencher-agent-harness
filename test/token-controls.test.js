...
test('context profile changes delegation bounds', async () => {
  ...
  const tight = await planTask(input, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('engineer'), context_profile: choice('tight'), retrieval_mode: choice('exact'), expand_context: noul(0), review_required: noul(0) }) })
...
test('low-confidence advice does not broaden deterministic policy', async () => {
  const plan = await planTask({ task: 'fix named file', rootCause: 'known', files: ['src/a.js'] }, { env: { TYPESAFE_API_KEY: 'x', JEV_MIN_CONFIDENCE: '.70' }, fetchImpl: jevFetch({ worker: choice('deep_debugger', .2), context_profile: choice('expanded', .2), retrieval_mode: choice('exploratory', .2), expand_context: noul(1), review_required: noul(0) }) })
...
test('Jev cannot escalate an evidence-free normal task', async () => {
  const plan = await planTask({ task: 'implement panel' }, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('deep_debugger'), context_profile: choice('normal'), retrieval_mode: choice('adjacent'), expand_context: noul(0), review_required: noul(0) }) })
  assert.equal(plan.route.role, 'engineer')
})
...
test('compact shared facts and inspection pointers are included in the Jev state', async () => {
  ...
  await askRoutingJev(createEvidencePacket({ task: 'follow up', facts: [{ claim: 'caller found', source: 'src/a.js:10' }], inspected: [{ path: 'src/a.js', range: '1-20', digest: 'abc' }] }), { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl })
...
test('unjustified expanded and exploratory advice is denied', async () => {
  const plan = await planTask({ task: 'fix named file', rootCause: 'known', files: ['src/a.js'] }, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('engineer'), context_profile: choice('expanded'), retrieval_mode: choice('exploratory'), expand_context: noul(1), review_required: noul(0) }) })
...
test('expandContext false caps exploratory retrieval at adjacent', async () => {
  const plan = await planTask({ task: 'investigate issue', openQuestions: ['Which direct dependency fails?'] }, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('engineer'), context_profile: choice('normal'), retrieval_mode: choice('exploratory'), expand_context: noul(0), review_required: noul(0) }) })
  assert.equal(plan.policy.retrievalMode, 'adjacent')
})

test('high-risk policy cannot be weakened by Jev', async () => {
  const plan = await planTask({ task: 'wallet settlement mismatch', risk: 'high' }, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('scout'), context_profile: choice('tight'), retrieval_mode: choice('exact'), expand_context: noul(0), review_required: noul(0) }) })
...
test('review and expandContext advice remain available', async () => {
  const plan = await planTask({ task: 'fix issue', rootCause: 'known' }, { env: { TYPESAFE_API_KEY: 'x' }, fetchImpl: jevFetch({ worker: choice('engineer'), context_profile: choice('normal'), retrieval_mode: choice('adjacent'), expand_context: noul(.4), review_required: noul(.8) }) })
...
test('successful provider execution records actual post-call usage', async () => {
  clearProviders()
  registerProvider('mock-usage', { execute: async query => normalizeResult({ provider: 'mock-usage', model: query.model, role: query.role, output: 'ok', usage: { inputTokens: 17, outputTokens: 4 } }) })
  const result = await executeDelegation({ delegation: { role: 'engineer', context: { task: 'bounded' }, instruction: 'work' }, provider: 'mock-usage', model: 'mock', budget: { maxEstimatedInputTokens: 1000 } })
